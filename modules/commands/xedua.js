...
module.exports.config = {
	name: "xedua", // Tên lệnh, được sử dụng trong việc gọi lệnh
...
module.exports.run = async ({ api, event, args, Users, Currencies, getText }) => {
	...
	switch (args[0]) {
		...
		default: {
        const picture = (await axios.get(`https://i.imgur.com/1ETPR1C.png`, { responseType: "stream"})).data
      return api.sendMessage({body: "===『 GAME XE ĐUA 』===\n\n♚─────────♔──────────♚\n» xedua info / xem thông tin xe \n» xedua shop / shop mua xe\n» xedua create / dua xe\n» xedua update / nâng cấp xe\n» xedua top / xem top server ",attachment: (picture)
