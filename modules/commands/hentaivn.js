...
module.exports.run = function({ api, event, args }) {
    ...
    return axios.get(`https://hentaivn.team/id${args[0]}`).then((response) => {
        if (response.status == 200) {
            const html = response.data;
            const $ = cheerio.load(html);
            var getContainer = $('div.container');
            var getURL = getContainer.find('form').attr('action');
            if (getURL == `https://hentaivn.team/${args[0]}-doc-truyen-.html`) return api.sendMessage(`Không tìm thấy truyện thông qua id bạn nhập :(`, event.threadID, event.messageID);
