...
const time = process.uptime(),
  ...
  const Canvas = require('canvas');
    ...
    let background = (await axios.get(encodeURI(`https://imgur.com/xF2uy9O.png`), { responseType: "arraybuffer" })).data;
...
let l1 = await loadImage(pathAva);
    ...
    ctx.font = "105px phenomicon";
    ctx.fillStyle = lengthchar[id].colorBg;
    ctx.fillText(global.config.BOTNAME, 850, 340);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/tad/UTM-Avo.ttf`, {
    ...
    ctx.fillText(`${z_1} : ${x_1} : ${y_1} `, 1000, 440);
    ctx.restore();
    ctx.save();
