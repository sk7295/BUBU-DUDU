...
const time = process.uptime(),
  ...
  const Canvas = require('canvas');
    ...
    let background = (await axios.get(encodeURI(`https://imgur.com/TjOFaRl.png`), { responseType: "arraybuffer" })).data;
...
let l1 = await loadImage(pathAva);
  ...
  ctx.drawImage(l1, 900, -5, 900, 1000);
     registerFont(__dirname + `/tad/DancingScript-VariableFont_wght.ttf`, {
      family: "phenomicon"
    });
    ctx.textAlign = "start";
    ...
    ctx.fillText("Nắng Mùa Thu", 360, 370);
    ctx.beginPath();
  ////////////////////////////////////////
   registerFont(__dirname + `/tad/DancingScript-VariableFont_wght.ttf`, {
      family: "UTM"
    ...
    ctx.fillText(`${z_1} : ${x_1} : ${y_1} `, 400, 470);
    ctx.restore();
    ctx.save();
registerFont(__dirname + `/tad/CaviarDreams.ttf`, {
    ...
    ctx.textAlign = "start";
    ctx.font = "45px time";
    //ctx.fillText("@" + "facebook.com/thinhvooo", 930, 590)
