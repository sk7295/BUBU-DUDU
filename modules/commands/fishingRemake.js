...
module.exports.checkPath = function (type, senderID) {
    ...
    const pathUser_1 = require("./FishingGlobal/datauser/" + senderID + '.json');
    const pathItem_1 = require("./FishingGlobal/item.json");
...
module.exports.onLoad = async () => {
    ...
    const dir = __dirname + `/FishingGlobal/`;
    const dirCache = __dirname + `/FishingGlobal/cache/`;
    const dirData = __dirname + `/FishingGlobal/datauser/`;
...
module.exports.run = async function ({
    ...
    const axios = require("axios")
    const pathData = path.join(__dirname, 'FishingGlobal', 'datauser', `${senderID}.json`);
    switch (args[0]) {
    case 'register':
    case '-r': {
    ...
    case 'top': {
        ...
        var p = []
        for (let i of data) { 
            var o = require(`./FishingGlobal/datauser/${i}`);
            p.push(o)
...
module.exports.dataFish =async function (a, b) {
    const data = require("./FishingGlobal/data.json");
...
module.exports.image = async function(link) {
    ...
    let download = (await axios.get(link, { responseType: "arraybuffer" } )).data; 
        fs.writeFileSync( __dirname + `/FishingGlobal/cache/fish.png`, Buffer.from(download, "utf-8"));
        images.push(fs.createReadStream(__dirname + `/FishingGlobal/cache/fish.png`));
    return images
}
module.exports.subnautica = async function() {
    ...
    let download = (await axios.get('https://i.imgur.com/kVPNSzw.jpg', { responseType: "arraybuffer" } )).data; 
        fs.writeFileSync( __dirname + `/FishingGlobal/cache/fishpage.png`, Buffer.from(download, "utf-8"));
        images.push(fs.createReadStream(__dirname + `/FishingGlobal/cache/fishpage.png`));
    return images
}
...
module.exports.handleReply = async function ({
    ...
    async function checkDur(a, b, c) {
        var data = require("./FishingGlobal/item.json");
