...
module.exports.config = {
  ...
  usages: "[new/leave/start/end],dùng lệnh theo hưỡng dẫn trên để chơi",
  cooldowns: 5
};
        ...
        if (q.choose.status == true) return api.sendMessage('𝗖𝗵𝗼̣𝗻 𝗿𝗼̂̀𝗶 𝗸𝗵𝗼̂𝗻𝗴 𝘁𝗵𝗲̂̉ 𝗰𝗵𝗼̣𝗻 𝗹𝗮̣𝗶!', threadID, messageID);
         ...
         if (body.toLowerCase() == 'tài') {
          ...
          gameThread.player.push({ name: q.name, userID: senderID, choose: { status: true, msg: 'tài' } });
            api.sendMessage({body:"「 𝗖𝗼𝗻 𝘇𝗼̛̣ " + q.name + " 𝗱𝗮̃ 𝗱𝗮̣̆𝘁 𝗰𝘂̛̉𝗮 𝗧𝗔̀𝗜 」\n𝗖𝗵𝘂́𝗰 𝗲𝗺 𝗺𝗮𝘆 𝗺𝗮̆́𝗻 𝘃𝗮̀ 𝗻𝗵𝗮𝗻𝗵 𝗰𝗵𝗼́𝗻𝗴 𝘃𝗲̂̀ 𝘃𝗼̛́𝗶 𝗰𝗮́𝘁 𝗯𝘂̣𝗶 𝗻𝗵𝗲́",attachment: createReadStream(__dirname + "/cache/bantaixiu/274209594_2862844057349581_3826390908571570035_n.jpg")},threadID, messageID);  
             }
       if (body.toLowerCase() == 'xỉu') {
            gameThread.player.splice(s, 1);
            gameThread.player.push({ name: q.name, userID: senderID, choose: { status: true, msg: 'xỉu' } });
            api.sendMessage({body:"「 𝗖𝗼𝗻 𝘇𝗼̛̣ " + q.name + " 𝗱𝗮̃ 𝗱𝗮̣̆𝘁 𝗰𝘂̛̉𝗮 𝗫𝗜̉𝗨 」\n𝗖𝗵𝘂́𝗰 𝗲𝗺 𝗺𝗮𝘆 𝗺𝗮̆́𝗻 𝘃𝗮̀ 𝗻𝗵𝗮𝗻𝗵 𝗰𝗵𝗼́𝗻𝗴 𝘃𝗲̂̀ 𝘃𝗼̛́𝗶 𝗰𝗮́𝘁 𝗯𝘂̣𝗶 𝗻𝗵𝗲́",attachment: createReadStream(__dirname + "/cache/bantaixiu/274182375_1308156316336656_5879771458750454501_n.jpg")},threadID, messageID); 
        }                            
          var vv = 0,
          ...
          if (vv == vvv) {
            api.sendMessage({body: "𝗗𝗮𝗻𝗴 𝗱𝗮̣̂𝗽 𝗮̀ 𝗸𝗵𝗼̂𝗻𝗴 𝗱𝗮𝗻𝗴 𝗹𝗮̆́𝗰 !!!", attachment: createReadStream(__dirname + "/cache/bantaixiu/274297685_2158259004350383_5647311325695705236_n.gif")},threadID,async  (err, data)  => { 
...
module.exports.run = async function({ api, event, args, Threads, Users, Currencies }) {
  try {
    ...
    if (args[0] == 'create' || args[0] == 'new' || args[0] == '-c') {
      ...
      if (!global.taixiuS.has(threadID)) return api.sendMessage('𝗛𝗶𝗲̣̂𝗻 𝘁𝗮̣𝗶 𝗸𝗵𝗼̂𝗻𝗴 𝗰𝗼́ 𝗯𝗮̀𝗻 𝗴𝗮𝗺𝗲 𝗻𝗮̀𝗼 𝗱𝗲̂̉ 𝗰𝗼́ 𝘁𝗵𝗲̂̉ 𝗿𝗼̛̀𝗶!', threadID, messageID);
      ...
      global.taixiuS.set(threadID, gameThread);
      return api.sendMessage({body: "◆𝗧𝗥𝗢̀ 𝗖𝗛𝗢̛𝗜 𝗕𝗔̆́𝗧 𝗗𝗔̂̀𝗨◆\n\n「 𝗫𝗶𝗻 𝗺𝗼̛̀𝗶 " + gameThread.player.length + " 𝗰𝗼𝗻 𝘇𝗼̛̣ 𝗻𝗴𝗵𝗶𝗲̣̂𝗻 𝗻𝗴𝗮̣̂𝗽 𝗰𝘂̉𝗮 𝗺𝗶̣ 𝗱𝗮̣̆𝘁 𝗰𝘂̛̉𝗮 𝗧𝗔̀𝗜 𝗵𝗼𝗮̣̆𝗰 𝗫𝗜̉𝗨 𝗱𝗲̂̉ 𝗰𝗵𝗼̛𝗶 」",attachment: createReadStream(__dirname + "/cache/bantaixiu/274221615_632733751117078_2633530530504954193_n.jpg")},threadID, messageID);
    }
